...
7. **合併系統更新 (重要!)**
   - 從 main 分支合併最新系統檔案
   - 執行: `git merge main --no-edit` 或 `git rebase main`
   - 處理可能的衝突:
     - 若 CLAUDE.md 或 dnd.md 有衝突,優先使用 main 的版本
     - 若 save.md 有衝突,保留角色分支的版本
   - 確認合併成功
...
從 `saves/[角色資料夾]/character.md` 讀取並顯示:
...
從 `saves/[角色資料夾]/equipment.md` 讀取裝備清單,提供以下操作:
...
檢查 `saves/` 資料夾中的存檔點:
- **只有當前存檔:** 提示無其他存檔點可讀取
- **有手動存檔點:** 使用 `AskUserQuestion` 顯示選擇清單
