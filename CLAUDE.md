...
當玩家輸入以 `系統訊息：` 開頭的訊息時,這**不是**對話推進或劇情選擇,而是要求執行系統性變更。
...
| 系統訊息範例 | 應執行的操作 |
|------------|------------|
| `系統訊息：角色名字誤植，應為 [正確名字]` | 修正名字後,重新生成上一段回應 |
| `系統訊息：剛才的擲骰結果有誤，應為 [正確數值]` | 套用正確數值,重新計算結果並重新敘事 |
| `系統訊息：這個 NPC 的性別應為女性` | 修正 NPC 設定,重新生成含該 NPC 的回應 |
| `系統訊息：場景應在夜晚，不是白天` | 修正場景設定,重新生成描述 |

### 注意事項

- ✅ 系統訊息優先於所有敘事邏輯
- ✅ 修正後應**無縫銜接**,不必特別說明「已修正」
- ❌ 不要將系統訊息當成角色對話或劇情輸入處理
- ❌ 不要繼續推進劇情再回頭修正,應直接重來
...
1. **難度選擇** ← 最先執行
   - 使用 `AskUserQuestion` 讓玩家選擇難度模式
   - 四個選項: 劇情 / 簡單 / 標準 / 困難
   - 說明各模式的規則 (詳見 `rules/04-character.md`)
   - **劇情模式:** 純敘事體驗,跳過屬性擲骰,冒險中無需檢定
   - 確認後才進入角色創建流程
...
6. **屬性擲骰**
   - **劇情模式:** 跳過此步驟
   - **其他模式:** 依步驟 1 選定的難度模式進行擲骰
     - 套用對應的上下界規則
     - 顯示結果後用 `AskUserQuestion` 詢問是否接受或整組重骰
     - 詳細規則見 `rules/04-character.md`
     - 根據職業特性分配屬性
...
### journal.md 與 meta.md 同步規則 (強制執行)

**每次存檔時，必須同時更新 journal.md 和 meta.md，確保兩者劇情進度一致。**

#### journal.md 自動壓縮機制

**觸發條件:** journal.md 超過 1000 行，或進入新的主要章節時

**壓縮規則:**
- **保留:** 最新章節（當前） + 上一個章節，維持完整記錄格式
- **壓縮:** 更早之前的所有章節 → 合併為「早期冒險回顧」區塊
- **格式:** 每個被壓縮的章節，保留 5~15 行的精簡摘要，確保重要事件、能力習得、人際關係、轉折點都有記錄
...
[5~10行精簡摘要，包含：重要事件、習得能力、關鍵NPC、劇情轉折]
...
**壓縮原則:**
- ✅ 保留所有「第一次」：第一次習得能力、第一次遇到重要 NPC、第一次重大事件
- ✅ 保留數值與技術細節：武器數據、魔法原理、重要設計
- ✅ 保留關係發展節點：好感度變化、重要對話
- ✅ 保留世界觀發現：新知識、新規律、新設定
- ❌ 刪除重複描述、冗長過場、細節對話（只保留結果）
