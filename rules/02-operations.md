...
長期冒險中，存檔資料會越來越龐大，消耗大量 token。以下規則定義何時壓縮、保留什麼、刪除什麼。
...
**步驟 1: 在 `journal.md` 留下簡短紀錄**
格式如下：
```
### ✅ [任務名稱] — 已完成 (第X天)
[一句話摘要：做了什麼、結果如何、重要影響]
```
範例：
```
### ✅ 救援艾莉絲 — 已完成 (第11天)
清除格林谷村西北哥布林營地，救出鐵匠之女艾莉絲，村莊對萊薇的態度轉為友好。
```

**步驟 2: 清除 `quests.md` 中該任務的詳細內容**
- 刪除任務的詳細描述、步驟、NPC 對話記錄
- 僅保留一行摘要供快速查閱：
  `- ✅ [任務名稱] (第X天完成)`

**步驟 3: 不刪除的內容**
- 任務相關的**具名 NPC** 保留在 `npcs.md`
- 任務給予的獎勵、裝備保留在 `equipment.md`
- 任務觸發的劇情謎團保留在 `notes.md`

---

### 離開城鎮後的壓縮

**觸發時機:** 角色確認離開某城鎮且短期內不會返回時

**執行步驟:**

**步驟 1: 壓縮 `location.md` 中的城鎮資料**

將詳細的城鎮描述（地圖、建築、服務、設施等）替換為簡要摘要：
...
| 等級 | 描述 | 影響 |
|------|------|------|
| ⭐⭐⭐⭐⭐ 英雄 | 被奉為救世主 | 免費住宿、最高折扣、任務優先 |
| ⭐⭐⭐⭐ 尊敬 | 廣受尊重 | 部分折扣、NPC 主動提供情報 |
| ⭐⭐⭐ 友好 | 一般好感 | 正常交易、任務可接 |
| ⭐⭐ 陌生 | 無特殊印象 | 正常交易，無額外優惠 |
| ⭐ 警戒 | 有不良記錄 | 部分 NPC 拒絕交易或提供情報 |
| ☆ 敵對 | 被列為通緝或仇敵 | 無法自由進入，可能遭受攻擊 |

**步驟 2: 具名 NPC 不刪除**
- 所有在 `npcs.md` 中有姓名的 NPC **永久保留**
- 僅新增一行標記：`[目前位於: 城鎮名稱]`
- 理由：具名 NPC 可能在未來重新出現、被提及、或影響劇情

**步驟 3: 匿名/背景 NPC 可刪除**
- 無名的「守衛」、「村民」、「商販」等背景角色
- 若無特殊劇情意義，可從 `npcs.md` 中移除
- 若玩家日後主動提起某個已刪除的匿名 NPC，以自然理由帶過：
  - 「那位守衛上個月換班了，換了新人。」
  - 「聽說他去北方城市找工作了。」
  - 「那個攤販最近沒來擺了，不知道去哪了。」
- 語氣自然，不刻意解釋，維持世界的流動感
...
| 具名 NPC | **永久保留** |
| 匿名背景 NPC | 可刪除 |
| 重大事件/謎團 | 移入 `notes.md` 保留 |
| 獎勵/道具 | 保留在 `equipment.md` |
| 日誌摘要 | 保留在 `journal.md` |
